...
#include "hq/athenea_probe_report.hpp"  // canonical AtheneaProbeReport definition
    ...
    handlers_["npu:athenea-probe"] = [](const C& cmd, CerberusRuntime& rt) -> CommandResult {
        ...
        struct RealQuantWeightDriver {
            ...
            void apply_to_compiled(hq::npu::CompiledKernel& ck) const {
                if (ck.inputs.empty()) {
                    ck.inputs.push_back(hq::npu::TensorDesc{"", {m_, k_}, hq::npu::TensorDesc::DataType::F32});
                    ck.inputs.push_back(hq::npu::TensorDesc{"", {k_, n_}, w_dtype_});
                }
                if (ck.outputs.empty()) {
                    ck.outputs.push_back(hq::npu::TensorDesc{"", {m_, n_}, hq::npu::TensorDesc::DataType::F32});
                }
            }
