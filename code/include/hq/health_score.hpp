...
struct HealthWeights {
    float gpu_utilization{0.15f};
    float hailo_utilization{0.15f};
    float npu_utilization{0.05f};
    float latency{0.20f};
    float memory{0.10f};
...
struct SubScores {
    ...
    float npu_utilization{0.0f};    // [0, 100] — pipeline NPU usage
...
struct RawHealthMetrics {
    float gpu_utilization_percent{0.0f};
    float gpu_temperature_c{0.0f};
    float hailo_utilization_percent{0.0f};
    float hailo_temperature_c{0.0f};
    float npu_utilization_percent{0.0f};
    float time_between_steps_ms{0.0f};
    float memory_bandwidth_percent{0.0f};
    float recovery_success_rate_percent{100.0f};
    float latency_drift_percent{0.0f};
};
...
struct HealthReport {
    ...
    RawHealthMetrics raw_metrics;
    std::string summary;            // e.g. "GPU 73% (A), TBT 32ms (A), thermal 72C (B)"
...
public:
    ...
    void update_npu_utilization(float util_pct);
...
private:
    ...
    std::atomic<float> npu_util_{0.0f};
    ...
    /// Build a human-readable summary string from raw metrics and sub-scores.
    [[nodiscard]] static std::string build_summary(const SubScores& sub,
